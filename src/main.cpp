...
#if defined(NDEBUG)
# error "Twitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3d4d61e5a4242ac7832cc9d51e0dfadfaf7fa374c69c2e5515d6ec68c34dbf3e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Twitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Twitcoin Signed Message:\n";
...
{
    // Twitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 33000000 * COIN;
    } else if (nHeight < 1337) {
        nSubsidy = 1 * COIN / 2;
    }
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Twitcoin: 1 days
static const int64 nTargetSpacing = .5 * 60; // Twitcoin: .5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Twitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Twitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8d4d3beb9a4507fb2e622e63119732723d1157788feb4d708cd1572b538d38e1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Secret Service to buy Harley-Davidson, despite Trump's support of boycotts";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0461e2e7f570e4d478e1f66a197532d89ad14aec38aa1a7d91f619ccfd0ada619fa42266bbec85461747c7239515540001446fe8a5c024067657887ec1bd13b2b4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1536895016;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2090689629;

        if (fTestNet)
        {
            block.nTime    = 1536894994;
            block.nNonce   = 389020308;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0ac466941d733c405fa7e20b072e72b2029948687d8484d61f7c25827e73c4dd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xbd, 0xd5 }; // Twitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TwitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TwitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TwitcoinMiner(CWallet *pwallet)
{
    printf("TwitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("twitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TwitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TwitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TwitcoinMiner, pwallet));
}
